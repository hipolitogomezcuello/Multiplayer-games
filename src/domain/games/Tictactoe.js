...
module.exports = class Tictactoe {
  constructor(lobby) {
    ...
    this.POSSIBLE_PLAYCOUNTS = [2];
  }

  markPlayerAsReadyToStart = (playerId) =>
    (this.players[playerId].readyToStart = true);
  isReadyToStart = () =>
    Object.keys(this.players).every(
      (key) => this.players[key].readyToStart === true
    );
  start = () => {
    const playersIds = Object.keys(this.players);
    Math.random() > 0.5
      ? (this.currentTurnPlayerId = playersIds[0])
      : (this.currentTurnPlayerId = playersIds[1]);
  ...
  makePlay = (playerId, cell) => {
    ...
    const nextTurnPlayerId = Object.keys(this.players).find(
      (playerId) => playerId !== this.currentTurnPlayerId
    );
    ...
    if (xMoves.length >= 3 || oMoves.length >= 3) {
      for (const winningCombo of winningCombos) {
        if (winningCombo.every((cell) => xMoves.includes(cell))) {
          this.winner = Object.keys(this.players).find(
            (key) => this.players[key].figure === "X"
          );
          this.currentTurnPlayerId = null;
          return;
        }
  ...
  playersWantToPlayAgain = () => {
    return Object.keys(this.players).every(
      (playerId) => this.players[playerId].wantsToPlayAgain
    );
  };
